...
| Capability | User value | v0.4.42 source status | Disposition |
| --- | --- | --- | --- |
...
- Compare now runs a separate decoded-signal worker that measures offset, gain, polarity, correlation, residual energy, and relationship. Stacked, overlay, wipe, and blink modes share zoom and horizontal navigation; aligned residual waveform/spectrogram panels and a selected-region measurement expose local differences without replacing full-track multichannel null evidence.
...
- Current-audit fingerprint matching uses exact-hash and ±3-second duration indexes instead of an all-pairs scan. Exact candidate expansion is capped and near-match expansion is deferred above 2,000 files. Historical enrichment uses no more than four outstanding storage requests, indexed exact-hash lookups above 100 files, and exact plus near-duration matching for smaller selections; it remains deferred above 1,000 files. A supplemental history timeout produces one audit notice and cannot discard completed Oracle verdicts or prevent session completion.
...
- The 168-test regression suite covers discovery, mounted-source staging and identity preservation, malformed metadata, PCM math, internal WAVE decoding, spectral-bin detection, multi-codec full decoding, FLAC audio-MD5 mismatch, loudness/true peak, delivery profiles, continuity, packet-rate behavior, fidelity controls, persistent fingerprint management, bounded 640-file historical finalization, indexed exact-history search, nonfatal enrichment timeout, AcoustID application-key normalization, service preflight, transient retry, cached-record identity enforcement, direct MusicBrainz enrichment, transient MusicBrainz throttling recovery, unique-ID caching, parallel identity corroboration/conflict classification, CD-database eligibility, neutral MQA marker inventory, privacy-safe acceptance evidence, decoded residual and selected-region comparison, desktop IPC error normalization, cue programme/pregap segments, calculated and large-library ReplayGain eligibility, explicit comparison channel mapping, resource enforcement, adaptive crash recovery with cache bypass, structured log rotation, cache invalidation, prompt cancellation of blocked workers, virtualized large-session viewport recovery, process cancellation, source-preserving repair output, attached-artwork retention, source preservation, truncation, and verdict truthfulness.
