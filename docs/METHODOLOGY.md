...
- AcoustID lookup is disabled by default. When the user supplies a registered-application key and explicitly enables it, Audio-V trims and locally validates the 10-character credential, then performs a service-backed preflight before discovery. A rejected key stops the audit instead of producing repeated per-file errors. Audio-V sends only the duration and Chromaprint value to AcoustID using POST and reads linked recording identifiers/titles. The key remains in memory and is removed from persisted sources and exported evidence. Requests have a 15-second timeout and a 1 MB response limit.
