...
**Audit resource policy** applies to the next audit. Choose 1–4 concurrent Oracle workers, a 128–512 MB JavaScript heap cap, 1/2/4 FFmpeg threads per file, and a 256 MB–2 GB native-process RSS limit. These selectors are per-file ceilings, so maximizing every selector is not a valid performance preset. Before creating workers, Audio-V caps their aggregate at the smaller of 20% of physical memory or 4 GB, reserves memory for the desktop process, and limits aggregate FFmpeg threads to 75% of logical CPUs. The Settings card previews the requested aggregate and the audit visibly reports any automatic reduction. Crossing an enforced native limit terminates that file as a resource error rather than calling it damaged.
