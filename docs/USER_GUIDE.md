...
Settings also displays the packaged Oracle validation basis: public independent external-reference count, source groups, controlled cases, corpus version, current claim level, and the first material limitation. “External corpus ready · references pending” means the validation machinery and five-source acquisition plan exist but no eligible external recording has been imported locally; it is not a hidden calibration score.
