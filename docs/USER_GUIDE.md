...
Settings also displays the packaged Oracle validation basis: public independent external-reference count, source groups, controlled cases, corpus version, scientific state, and the first material limitation. “External corpus ready · references pending” means the validation machinery and five-source acquisition plan exist but no eligible external recording has been imported locally; it is not a hidden calibration score. “Evaluated · no model promoted” means the corpus evaluation finished, but the experimental Origin candidate failed its independent false-advisory requirements. It is not a background task, and ordinary audits never train or silently recalibrate Oracle.
